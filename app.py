...
def show_result_dialog(report_text, fio_name):
    ...
    # 1. ОСНОВНОЕ ПОЛЕ (Тут иконка копирования в правом верхнем углу!)
    st.text_area("Текст заключения (иконка копирования справа ↗️):", report_text, height=400)
    ...
    with col1:
        ...
        bio = io.BytesIO()
        doc.save(bio)
        st.download_button("📥 СКАЧАТЬ .DOCX", bio.getvalue(), f"{fio_name}.docx", use_container_width=True)
        
    with col2:
        # Кнопка-заглушка для закрытия (или просто инфо)
        if st.button("❌ ЗАКРЫТЬ ОКНО", use_container_width=True):
            st.rerun()

    st.info("💡 Лайфхак: На смартфоне нажми на текст и выбери 'Копировать' или используй иконку в углу поля.")
